...
void printForSort(FILE *file, int num, Recipe *dishes){
    for(int i = 0; i < num; i++) {
        cout << std::left << setw(50) << dishes[i].name;
        cout << std::left << setw(10) << dishes[i].type;
        cout << std::left << setw(10) << dishes[i].time;
        cout << std::left << setw(10) << dishes[i].kall;
        cout << std::left << setw(100) << dishes[i].description << endl;
...
void printMessage(){
    cout << "\n 1 - add recipe(s).\n 2 - edit recipe.\n 3 - delete recipe.\n\n "
            "4 - sorts: \n\t4.1 - sort by category \n\t4.2 - сортировка по названию (пузырьком) \n\t4.3 - calories (quick sort) \n\t4.4 - time (selection sort)\n\t4.5 - exit\n\n"
            "5 - search: \n\t5.1 - name (linear) \n\t5.2 - calories (interpolated)\n\t5.3 - exit\n\n"
            "6 - search diet recipe.\n"
            "7 - menu\n"
            "8 - exit the program\n";
}
